...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a Sutton-first-author papers CSV from cached arXiv API XML files."
    )
    parser.add_argument(
        "--author-dir",
        default="AUTHORS/Sutton-Richard",
        help="Author directory containing API/page-*.xml files (default: AUTHORS/Sutton-Richard).",
    )
    parser.add_argument(
        "--output",
        default="papers-sutton-richard-first-author.csv",
        help="Output CSV path (default: papers-sutton-richard-first-author.csv).",
    )
...
def is_richard_sutton(name: str) -> bool:
...
def first_author_is_sutton(entry: ET.Element) -> bool:
...
def collect_papers(author_dir: Path) -> tuple[list[Paper], int]:
    ...
    for xml_path in iter_api_files(author_dir):
        ...
        for entry in root.findall("atom:entry", NAMESPACES):
            if not first_author_is_sutton(entry):
                continue
